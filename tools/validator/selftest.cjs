...
// ── #3 文法 ──
t('#3 when 文法非法 → E103', base({}, { order: [{ from: 'a', to: 'b', when: 'signal.go ==', level: 2 }] }), {}, ['ACT-E103'])
...
t('#15 level:1 用 signal → E104',
  base({}, { order: [{ from: 'a', to: 'b', when: 'signal.go == true', level: 1 }] }), {}, ['ACT-E104'])
...
// ── #5 引用存在性 ──
t('#5 工具不存在 → E105',
  base({ a: { tools: ['no_such_tool'] } }), { toolRegistry: new Set(['docx_write']) }, ['ACT-E105'])

t('#5 $ref 无法解析 → E105',
  base({}, { amz: [{ $ref: 'amz/missing' }] }), {}, ['ACT-E105'])

t('#5 UI 页面组不存在 → E105',
  base({}, { ui: { page: ['AGT', 'WFW', 'DIR', 'GHOST'] } }), { uiPages: new Set(['AGT', 'WFW', 'DIR', 'WPC']) }, ['ACT-E105'])

// ── #6 signal 字段 ──
t('#6 signal 字段未声明 → E106',
  base({}, { order: [{ from: 'a', to: 'b', when: 'signal.ghost == true', level: 2 }] }), {}, ['ACT-E106'])

t('#6 signal 类型不符 → E106',
  base({}, { order: [{ from: 'a', to: 'b', when: "signal.go == 'yes'", level: 2 }] }), {}, ['ACT-E106'])
...
t('#7 入口不唯一 → E107',
  ...
  ], terminal: ['b', 'd'] }), {}, ['ACT-E107'])

// ── #8 终态 ──
t('#8 非终态无出边 → E108', base({}, { terminal: ['a'] }), {}, ['ACT-E108'])
t('#8 terminal 有出边 → W204', base({}, { terminal: ['a', 'b'] }), {}, ['ACT-W204'])

// ── #9 导入复核 ──
t('#9 import 无 _review → E109', base(), { mode: 'import' }, ['ACT-E109'])

{
  const swf = base()
  const surface = [{ amz: 'a', tools: [], guards: [] }, { amz: 'b', tools: [], guards: [] }]
  const good = surfaceHash(surface)
  t('#9 import 哈希正确 → 无 E109', Object.assign(base(), { _review: { at: 'x', by: 'y', surfaceHash: good } }), { mode: 'import' }, [], ['ACT-E109'])
  t('#9 import 哈希不符 → E109', Object.assign(base(), { _review: { at: 'x', by: 'y', surfaceHash: 'sha256:deadbeef' } }), { mode: 'import' }, ['ACT-E109'])
}

// ── #10 step ──
t('#10 存在 step → W205',
  base({ b: { kind: 'step', step: { from: 'a', scp: 'x' }, output: { body: 'text' } } }), {}, ['ACT-W205'])

// ── #11 CSP（schema 层已实测；此处只确认不误报）──
t('#11 正常 AMZ 不报任何 CSP 码', base(), {}, [], ['ACT-E001'])
...
t('#12 AMZ id 重复 → E110',
  base({}, { amz: [
    { id: 'a', kind: 'exp', prompt: 'p', tools: [], output: { body: 'text' } },
    { id: 'a', kind: 'exp', prompt: 'p', tools: [], output: { body: 'text' } },
  ], order: [{ from: 'a', to: 'a', when: 'run.status == "ok"', level: 1 }], terminal: ['a'] }), {}, ['ACT-E110'])

// ── #13 分支出边缺 signal ──
t('#13 level:2 出边但无 output.signal → E111',
  base({ a: { output: { body: 'free' } } }), {}, ['ACT-E111'])
...
// ── #1 工具面安全 ──
t('#1 含 exec 类工具 → W201', base({ a: { tools: ['pwsh'] } }), {}, ['ACT-W201'])
t('#1 exec + artifact 并存 → W201 + W206', base({ a: { tools: ['pwsh', 'docx_write'] } }), {}, ['ACT-W201', 'ACT-W206'])
